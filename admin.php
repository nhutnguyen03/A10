...
<body>
    ...
    // Tạo đối tượng để làm việc với dữ liệu
    $hdBUS = new HoaDonBUS();
    ...
    // Bắt đầu tạo nội dung bảng trong table-content
    echo "<table class='table-content'>";

    // Lấy tất cả các hóa đơn
foreach ($hdBUS->select_all() as $hoaDon) {
    // Lấy thông tin khách hàng (người dùng) dựa trên MaND
    $nguoiDung = $ndBUS->select_by_id("*", $hoaDon['MaND']);
    $tenKhach = $nguoiDung['Ho'] . " " . $nguoiDung['Ten'];
    
    // Lấy danh sách các chi tiết hóa đơn
    $chiTietHoaDon = $cthdBUS->select_all_in_hoadon($hoaDon['MaHD']);
    
    // Tạo chuỗi để hiển thị danh sách sản phẩm
    $dsSanPham = "";
    foreach ($chiTietHoaDon as $ct) {
        // Lấy thông tin sản phẩm dựa trên MaSP
        $sanPham = $spBUS->select_by_id("*", $ct['MaSP']);
        if ($sanPham) {
            $dsSanPham .= $sanPham['TenSP'] . " (x" . $ct['SoLuong'] . "), ";
        } else {
            $dsSanPham .= "Sản phẩm không tìm thấy (x" . $ct['SoLuong'] . "), ";
        }
    }
    // Xóa dấu phẩy cuối cùng
    $dsSanPham = rtrim($dsSanPham, ", ");

    // Hiển thị thông tin hóa đơn
    echo "<tr>
        <td style='width: 5%'>" . $i++ . "</td>
        <td style='width: 13%'>" . $hoaDon['MaHD'] . "</td>
        <td style='width: 7%'>" . $tenKhach . "</td>
